...
def test_fetch_weather_data_success():
    ...
    mock_data = {
        "properties": {
            "timeseries": [
                {"time": "2024-03-15T10:00:00Z"} for _ in range(10)
            ]  # Sufficient data
        }
    }
...
def test_fetch_weather_data_complete_endpoint_empty_timeseries():
    ...
    empty_data = {"properties": {"timeseries": []}}
...
def test_fetch_weather_data_both_endpoints_insufficient():
    """Test when both complete and compact endpoints return insufficient data."""
    location = Location("test", "Test", 40.0, -3.0)

    insufficient_data = {
        "properties": {
            "timeseries": [{"time": "2024-03-15T10:00:00Z"}]
        }
    }
...
def test_fetch_weather_data_complete_endpoint_null_properties():
    """Test fetch_weather_data when complete endpoint returns properties as None."""
    location = Location("test", "Test", 40.0, -3.0)

    # Mock data with null properties
    null_properties_data = {"properties": None}
